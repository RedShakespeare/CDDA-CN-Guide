...
class ExportMD:
    ...
    def get_UserInfo(self):
        ...
        if os.path.isfile(f_name):
            ...
            namespace = 'silencess'
            Token = 'vEIDBlJDwKltqMrkR7pC3Y5wRGMAoQWiuUmwfqkP'
    ...
    async def download_md(self, repo_id, slug, repo_name, title):
        ...
        if image_list:
            # 图片保存位置: ../<repo_name>/media/<filename>
            save_dir = os.path.join(self.export_dir, repo_name, "media")
            self.mkDir(save_dir)
            async with aiohttp.ClientSession() as session:
    ...
    def save(self, repo_name, title, body):
        ...
        save_path = "../%s/%s.md" % (repo_name, title)
